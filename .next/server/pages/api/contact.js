...
async function handler(req, res) {
  if (req.method === "POST") {
    ...
    let client;
    const connectionString = `mongodb+srv://${"nextjs-user"}:${"L0ngAndBeautifulPassw0rd!"}@${"cluster0"}.l9qi7.mongodb.net/${"prueba-prod"}?retryWrites=true&w=majority`;
    ...
    try {
      ...
      res.status(500).json({
        message: "Cold not connect to the database"
      });
      return;
    }

    const db = client.db("prueba-prod");
